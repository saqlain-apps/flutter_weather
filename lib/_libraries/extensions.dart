...
extension ExtendedList<E> on List<E> {
  ...
  List<E> distinctBy<T>(T Function(E element) identity) {
    return Map.fromEntries(map((e) => MapEntry<T, E>(identity(e), e)))
        .values
        .toList();
  }
...
extension ExtendedString on String {
  ...
  String get snakeCase {
    var result = replaceAllMapped(
        RegExp(r'([A-Z])'), (match) => ' ${match.groups([1]).first!}');
    result = result.trim();
...
extension ExtendedColor on Color {
  ...
  WidgetStateProperty<Color> get statePropertyColor =>
      WidgetStateProperty.all<Color>(this);
}
