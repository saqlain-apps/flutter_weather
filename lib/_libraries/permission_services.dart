...
class PermissionServices {
    ...
    switch (requestResource) {
      ...
      case RequestResource.storage:
        if (Platform.isIOS) {
          ...
          return await checkMultiPermission(
  ...
  static Future<bool> checkMultiPermission(List<Permission> permission) async {
    final status = combineStatus((await permission.request()).values);
    return await processRequest(
      status,
    ...
    return status.reduce((value, element) {
      if (value == element) {
        ...
        return PermissionStatus.permanentlyDenied;
          ...
          element == PermissionStatus.restricted) {
        return PermissionStatus.restricted;
      } else if (value == PermissionStatus.provisional ||
          element == PermissionStatus.provisional) {
